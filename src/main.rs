...
use gnuplot::{AxesCommon, Caption, Color};
    ...
    F: Fn(&mut [A]),
{
    ...
    for _ in 0..r {
        x.push(Vec::new());
        for _ in 0..n {
            let a = A(rng.sample(Uniform::new(i32::MIN, i32::MAX)));
            x.last_mut().unwrap().push(a);
        }
    }
    ...
    (elapsed / r as f64, ncmp / r, ncpy / r)
}
...
fn chain_bench<F>(
    ...
    rep: usize,
    f: F,
) -> (Vec<usize>, Vec<f64>, Vec<usize>, Vec<usize>)
where
    F: Fn(&mut [A]),
{
    let mut ns = Vec::new();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    {
        let a_time = fg_time.axes2d().set_title("time", &[]);
        let a_cmp = fg_cmp.axes2d().set_title("compare", &[]);
        let a_cpy = fg_cpy.axes2d().set_title("copies", &[]);

        let (ns, ts, cmps, cpys) = chain_bench(nmax, nstep, rep, quick::quick_sort);
        ...
        let (ns, ts, cmps, cpys) = chain_bench(nmax, nstep, rep, heap::heap_sort);
        ...
        let (ns, ts, cmps, cpys) = chain_bench(nmax, nstep, rep, merge::merge_sort);
        ...
        let (ns, ts, cmps, cpys) = chain_bench(nmax, nstep, rep, |x| x.sort());
